...
#===============================================================================================
#   frp a fast reverse proxy Installer Script (v3 - Removed Web Server)
#
...
#   - Sets up a systemd service for frps (without web dashboard).
#
#   Usage:
...
main() {
    ...
    if [ -z "$LATEST_VERSION" ]; then
        error "Failed to fetch the latest frp version."
    fi
...
EOF
    ...
    echo "1. Edit the server configuration file if needed:"
    echo "   sudo vim $CONFIG_DIR/frps.toml"
    echo
    echo "2. Securely upload your certificates to '$CONFIG_DIR/'"
    echo "   (See next steps on how to do this with Termius or scp)"
    echo
    echo "3. After uploading certs, start the frps service:"
